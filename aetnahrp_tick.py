"""
Combined Aetna HRP tick — mirrors aetna_tick.py (the RCE one) but SUPPORT-ONLY.

The Windows task "AetnaHRP Tick" runs this every 2 hours, 6am-4pm. Each tick:
  - ALWAYS runs the fail-only monitor (ramp_aetnahrp_slack_monitor.py). Any
    HRP-FAILED alert is re-emitted as 'POST_SUPPORT|<text>' -> post to
    #team-rdp-operations-support (C09EPLQL2D9), then --commit.
  - At a DIGEST SLOT (hour 8/12/16, Tue-Fri) also runs the status digest
    (ramp_aetnahrp_status_digest.py). Its single line is re-emitted as
    'POST_DIGEST|<text>' -> post to the SAME support channel (NO mining
    channel, per user). The digest's own dedupe + succeeded-skip still apply.
    Digest needs no commit (it self-dedupes).
...
PY = sys.executable
MONITOR = BASE + r'\ramp_aetnahrp_slack_monitor.py'
DIGEST = BASE + r'\ramp_aetnahrp_status_digest.py'
