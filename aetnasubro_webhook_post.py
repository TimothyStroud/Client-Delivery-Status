"""
Headless Aetna Subro poster (zero Claude tokens). Mirrors aetnarx_webhook_post.py.
All posts route to #data-operations-aetna-updates, the same channel as the RCE /
HRP / Rx monitors.

Runs aetnasubro_tick.py (fail-only monitor every tick + status digest at slots)
and posts its tagged output to Slack via the aetna-updates Workflow webhook:
  POST_SUPPORT|<text>  -> aetna-updates channel  (Subro load failure alert)
  POST_DIGEST|<text>   -> aetna-updates channel  (status digest; no commit)
After a POST_SUPPORT posts OK, runs `aetnasubro_tick.py --commit` (two-phase, so a
...
sanitize() strips *, `, blockquote '> ', and <!here> (emoji kept). @here does NOT
ping (accepted, matches the other Aetna monitors).
...
PY = sys.executable
TICK = os.path.join(BASE, 'aetnasubro_tick.py')
AETNA_URL_FILE = r'H:\slack_wf_aetna_updates.txt'
LOG_FILE = r'H:\aetnasubro_webhook_post.log'
...
def sanitize(text):
    ...
    renders), so strip them. <!here> also shows literally + doesn't ping."""
...
def main():
    ...
    r = subprocess.run([PY, TICK], capture_output=True, text=True, timeout=600)
    ...
    for line in r.stdout.splitlines():
        if line.startswith('POST_SUPPORT|'):
            ...
            try:
                post(updates, txt)
                posted_support = True
                log("posted SUBRO FAILURE -> aetna-updates")
