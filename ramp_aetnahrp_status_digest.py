...
def hrp_run():
    """Return the LatestJobRun dict for RAMP 'Aetna 0110 HRP Load' (1246)."""
...
def main():
    ...
    if (hrp_succeeded_today()
        ...
        print('NO_POST: HRP load Succeeded today (RAMP + SQL)')
        return
    now = datetime.now().strftime('%m/%d/%Y %I:%M %p')
    lines = [f"<!here> :bar_chart: *Aetna 0110 HRP Load - Status Update*  ({now})", ""]
    lines.append("*RAMP - Aetna 0110 HRP Load*")
    lines.append(hrp_status())
    lines.append("")
